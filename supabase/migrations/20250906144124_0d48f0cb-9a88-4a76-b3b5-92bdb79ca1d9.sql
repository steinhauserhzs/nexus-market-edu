-- Simple approach: Create demo store with existing user or temporarily allow null
DO $$ 
DECLARE
    store_id_var UUID;
...
BEGIN
    ...
    -- If doesn't exist, create demo data
    IF store_id_var IS NULL THEN
        -- Try to get an existing user (any seller)
        SELECT id INTO existing_user_id FROM public.profiles WHERE role = 'seller' LIMIT 1;
        
        -- If no user exists, we'll skip foreign key (set as NULL)
